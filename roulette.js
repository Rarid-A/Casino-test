...
function placeRouletteBet(event) {
    ...
    const intervalTime = 70; // Slower animation (100ms between highlights)
    const totalSpins = Math.floor(spinDuration / intervalTime); // Total iterations
    const extraSpins = cells.length * 2; // Extra spins to make it look more dynamic
    const finalIndex = spinResult + extraSpins; // Ensure it lands on the correct number
